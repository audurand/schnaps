...
/*!
 * \brief Construct an operator to add right argument from left argument and store result in a variable as a copy of an original.
 * \param inOriginal A const reference to the original operator to add right argument from left argument and store result in a variable.
 */
 ...
 */
AddVariable& AddVariable::operator=(const AddVariable& inOriginal) {
	...
	return *this;
	schnaps_StackTraceEndM("SCHNAPS::Plugins::Operators::AddVariable& SCHNAPS::Plugins::Operators::AddVariable::operator=(const SCHNAPS::Plugins::Operators::AddVariable&)");
}
...
Core::AnyType::Handle AddVariable::execute(unsigned int inIndex, Core::ExecutionContext& ioContext) const {
	...
	if (mResult_Ref[0] == '@') {
		...
		lSimulationContext.getIndividual().getState().setVariable(mResult_Ref.substr(1), lArgLeft->add(*lArgRight));
	} else { // mResult_Ref[0] == '%'
		// local variable
		lContext.setLocalVariable(mResult_Ref.substr(1), lArgLeft->add(*lArgRight));
	}
