 ...
 * \throw  SCHNAPS::RunTimeException if the primitive is undefined for the specific value source.
 */
	...
	if (mValue == NULL) {
		switch (mValue_Ref[0]) {
			...
			case '%':
				...
				break;
			default:
				throw schnaps_RunTimeExceptionM("The primitive is undefined for the specific value source.");
				break;
		}
	} else {
		// parameter value or direct value
