...
Pow& Pow::operator=(const Pow& inOriginal) {
	schnaps_StackTraceBeginM();
		return *this;
	schnaps_StackTraceEndM("Core::Pow& SCHNAPS::Plugins::Operators::Pow::operator=(const Core::Pow&)");
}
...
Core::AnyType::Handle Pow::execute(unsigned int inIndex, Core::ExecutionContext& ioContext) const {
	schnaps_StackTraceBeginM();
		...
		lArg1->pow(*lArg2);
		return lArg1;
	schnaps_StackTraceEndM("Core::AnyType::Handle SCHNAPS::Plugins::Operators::Pow::execute(unsigned int, Core::ExecutionContext&) const");
}
...
const std::string& Pow::getArgType(unsigned int inIndex, unsigned int inN, Core::ExecutionContext& ioContext) const {
	...
	schnaps_StackTraceEndM("const std::string& SCHNAPS::Plugins::Operators::Pow::getArgType(unsigned int , unsigned int, Core::ExecutionContext&) const");
}
...
const std::string& Pow::getReturnType(unsigned int inIndex, Core::ExecutionContext& ioContext) const {
	schnaps_StackTraceBeginM();
		unsigned int lNodeIndex = getArgumentIndex(inIndex, 0, ioContext);
		return ioContext.getPrimitiveTree()[lNodeIndex].mPrimitive->getReturnType(inIndex, ioContext);
	schnaps_StackTraceEndM("const std::string& SCHNAPS::Plugins::Operators::Pow::getReturnType(unsigned int, Core::ExecutionContext&) const");
}
