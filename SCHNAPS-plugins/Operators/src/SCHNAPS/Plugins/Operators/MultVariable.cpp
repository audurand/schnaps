...
Core::AnyType::Handle MultVariable::execute(unsigned int inIndex, Core::ExecutionContext& ioContext) const {
	...
	if (mResult_Ref[0] == '@') {
		...
		lSimulationContext.getIndividual().getState().setVariable(mResult_Ref.substr(1), lArgLeft->mult(*lArgRight));
	} else { // mResult_Ref[0] == '%'
		// local variable
		lContext.setLocalVariable(mResult_Ref.substr(1), lArgLeft->mult(*lArgRight));
	}
...
const std::string& MultVariable::getReturnType(unsigned int inIndex, Core::ExecutionContext& ioContext) const {
	...
	return lType;
	schnaps_StackTraceEndM("const std::string& SCHNAPS::Plugins::Operators::MultVariable::getReturnType(unsigned int, SCHNAPS::Core::ExecutionContext&) const");
}
