		...
		mKeyType(inOriginal.mKeyType.c_str())
{
	...
	for (SwitchMap::const_iterator lIt = inOriginal.mSwitchMap.begin(); lIt != inOriginal.mSwitchMap.end(); lIt++) {
		mSwitchMap.insert(std::pair<SCHNAPS::Core::Atom::Handle, unsigned int>(SCHNAPS::Core::castHandleT<SCHNAPS::Core::Atom>(lIt->first->clone()), lIt->second));
	}
}

void Switch::readWithSystem(PACC::XML::ConstIterator inIter, SCHNAPS::Core::System& ioSystem) {
	schnaps_StackTraceBeginM();
		...
		if (inIter->getAttribute("keys").empty()) {
			if (inIter->getAttribute("keys.ref").empty()) {
				...
				SCHNAPS::Core::Vector::Handle lKeys = SCHNAPS::Core::castHandleT<SCHNAPS::Core::Vector>(ioSystem.getParameters()[lSS.str().c_str()]);
				for (unsigned int i = 0; i < lKeys->size(); i++) {
					mSwitchMap.insert(std::pair<SCHNAPS::Core::Atom::Handle, unsigned int>(SCHNAPS::Core::castHandleT<SCHNAPS::Core::Atom>((*lKeys)[i]), i));
				}
				lChoice = lKeys->size();
			}
			...
			SCHNAPS::Core::Atom::Alloc::Handle lAlloc = SCHNAPS::Core::castHandleT<SCHNAPS::Core::Atom::Alloc>(ioSystem.getFactory().getAllocator(mKeyType));
			...
			std::string lKey;
			SCHNAPS::Core::Atom::Handle lAtom;

			lChoice = 0;
			while (lTokenizer.getNextToken(lKey)) {
				// add to map
				lAtom = SCHNAPS::Core::castHandleT<SCHNAPS::Core::Atom>(lAlloc->allocate());
				if (lAtom == NULL) {
					std::ostringstream lOSS;
				...
				lAtom->readStr(lKey);
				mSwitchMap.insert(std::pair<SCHNAPS::Core::Atom::Handle, unsigned int>(lAtom, lChoice));
				lChoice++;
			}
		}
...
void Switch::writeContent(PACC::XML::Streamer& ioStreamer, bool inIndent) const {
	schnaps_StackTraceBeginM();
		if (mKeys_Ref.empty()) {
			std::string lKeys;
			for (std::map<SCHNAPS::Core::Atom::Handle, unsigned int, SCHNAPS::Core::IsLessPointerPredicate>::const_iterator lIt = mSwitchMap.begin(); lIt != mSwitchMap.end(); lIt++) {
...
SCHNAPS::Core::AnyType::Handle Switch::execute(unsigned int inIndex, SCHNAPS::Core::ExecutionContext& ioContext) const {
	schnaps_StackTraceBeginM();
		SCHNAPS::Core::Atom::Handle lArg = SCHNAPS::Core::castHandleT<SCHNAPS::Core::Atom>(getArgument(inIndex, 0, ioContext));
...
const std::string& Switch::getArgType(unsigned int inIndex, unsigned int inN, SCHNAPS::Core::ExecutionContext& ioContext) const {
	schnaps_StackTraceBeginM();
	schnaps_AssertM(inN<getNumberArguments());
		if (inN == 0) {
			if (mKeys_Ref.empty()) {
				...
				SCHNAPS::Core::Vector::Handle lKeys = SCHNAPS::Core::castHandleT<SCHNAPS::Core::Vector>(ioContext.getSystem().getParameters()[lSS.str().c_str()]);
				if (lKeys->size() < 1) {
...
const std::string& Switch::getReturnType(unsigned int inIndex, SCHNAPS::Core::ExecutionContext& ioContext) const {
	schnaps_StackTraceBeginM();
