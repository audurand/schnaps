 ...
 * \param inPop A handle to the individuals.
 */
void Generator::generateContacts(Individual::Bag::Handle inPop){
	...
	//backup generation randomizer, load simulation randomizer
	...
	schnaps_StackTraceEndM("void SCHNAPS::Simulation::Generator::generateContacts(SCHNAPS::Simulation::Individual::Bag::Handle inPop)");
}
...
printf("Reading randomizer info\n");
	...
	if (inIter->getChildCount() > 0) {
	    if (inIter->getChildCount() < lThreadsGenerator) {
        ...
        for (unsigned int i = 0; i < lThreadsGenerator; i++) {
