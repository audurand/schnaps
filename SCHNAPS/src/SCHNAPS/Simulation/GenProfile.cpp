...
GenProfile::GenProfile(Demography::Handle inDemography, SCHNAPS::Core::PrimitiveTree::Handle inAcceptFunction, IndividualModel::Handle inIndividualModel, SimulationVariables::Handle inSimulationVariables) :
...
void GenProfile::readWithSystem(PACC::XML::ConstIterator inIter, SCHNAPS::Core::System& ioSystem) {
...
GenProfile::Handle GenProfile::deepCopy(const SCHNAPS::Core::System& ioSystem) const {
	schnaps_StackTraceBeginM();
		...
		for (unsigned int i = 0; i < this->mDemography->size(); i++) {
			lGenProfile->mDemography->push_back(std::pair<std::string, SCHNAPS::Core::PrimitiveTree::Handle>((*this->mDemography)[i].first.c_str(), (*this->mDemography)[i].second->deepCopy(ioSystem)));
		}
		...
		for (unsigned int i = 0; i < this->mSimulationVariables->size(); i++) {
			lGenProfile->mSimulationVariables->push_back(std::pair<std::string, SCHNAPS::Core::PrimitiveTree::Handle>((*this->mSimulationVariables)[i].first.c_str(), (*this->mSimulationVariables)[i].second->deepCopy(ioSystem)));
		}
