		...
		mBlackBoardWrt(new PACC::Threading::Semaphore(1))
{
	...
	mSystem->getFactory().insertAllocator("State", new simulator::State::Alloc());
	mSystem->getFactory().insertAllocator("Individual", new simulator::Individual::Alloc());
	mSystem->getFactory().insertAllocator("Environment", new simulator::Environment::Alloc());
	mSystem->getFactory().insertAllocator("Population", new simulator::Population::Alloc());
	mSystem->getFactory().insertAllocator("Generator", new simulator::Generator::Alloc());
	mSystem->getFactory().insertAllocator("Process", new simulator::Process::Alloc());
	mSystem->getFactory().insertAllocator("Clock", new simulator::Clock::Alloc());

	// Initialize required parameters
	mSystem->getParameters().addParameter("print.prefix", new core::String(""));
	mSystem->getParameters().addParameter("print.input", new core::Bool(false));
	mSystem->getParameters().addParameter("print.output", new core::Bool(true));
	mSystem->getParameters().addParameter("print.log", new core::Bool(true));
	mSystem->getParameters().addParameter("print.conf", new core::Bool(false));
	mSystem->getParameters().addParameter("threads.simulator", new core::UInt(1));
	mSystem->getParameters().addParameter("threads.generator", new core::UInt(1));
}
...
void Simulator::read(PACC::XML::ConstIterator inIter) {
	schnaps_StackTraceBeginM();
		...
		readOutput(inIter);
	schnaps_StackTraceEndM("void simulator::Simulator::read(PACC::XML::ConstIterator)");
}
...
void Simulator::configure(const std::string& inParameters) {
	schnaps_StackTraceBeginM();
		...
		// Initialize simulator
		init();
	schnaps_StackTraceEndM("void simulator::Simulator::configure(const std::string&)");
}

std::string Simulator::getConfiguration() {
	schnaps_StackTraceBeginM();
		return mSystem->getParameters().writeStr();
	schnaps_StackTraceEndM("std::string simulator::Simulator::getConfiguration()");
}
...
void Simulator::simulate(std::string inScenarioLabel) {
	schnaps_StackTraceBeginM();
		...
		// Print info
		std::string lPrintPrefix = core::castHandleT<core::String>(mSystem->getParameters()["print.prefix"])->getValue();
		bool lPrintInput = core::castHandleT<core::Bool>(mSystem->getParameters()["print.input"])->getValue();
		bool lPrintOutput = core::castHandleT<core::Bool>(mSystem->getParameters()["print.output"])->getValue();
		bool lPrintLog = core::castHandleT<core::Bool>(mSystem->getParameters()["print.log"])->getValue();
		ogzstream lOGZS;
	...
	schnaps_StackTraceEndM("void simulator::;Simulator::simulate(std::string, std::string, std::string)");
}
...
void Simulator::clearRandomizer() {
	...
	schnaps_StackTraceEndM("void simulator::Simulator::clearRandomizer()");
}
...
void Simulator::resetRandomizer() {
	...
	schnaps_StackTraceEndM("void simulator::Simulator::resetRandomizer()");
}
...
void Simulator::processScenario(SimulationThread* inThread) {
	...
	schnaps_StackTraceEndM("void simulator::Simulator::processScenario(simulator::Thread*)");
}
...
void Simulator::processClockStep(SimulationThread* inThread) {
	...
	schnaps_StackTraceEndM("void simulator::Simulator::processClockStep(simulator::Thread*)");
}
...
void Simulator::processSubStep(SimulationThread* inThread) {
	...
	schnaps_StackTraceEndM("void simulator::Simulator::processSubStep(simulator::Thread*)");
}
...
void Simulator::readSimulation(PACC::XML::ConstIterator inIter) {
	...
	schnaps_StackTraceEndM("void simulator::Simulator::readSimulation(PACC::XML::ConstIterator)");
}
...
void Simulator::readOutput(PACC::XML::ConstIterator inIter) {
	...
	schnaps_StackTraceEndM("void simulator::Simulator::readOutput(PACC::XML::ConstIterator)");
}
...
void Simulator::readRandomizerInfo(PACC::XML::ConstIterator inIter) {
	...
	schnaps_NonNullPointerAssertM(mSystem);
		...
		unsigned int lThreadsSimulator = core::castHandleT<core::UInt>(mSystem->getParameters()["threads.simulator"])->getValue();
		...
		mRandomizerCurrentSeed = mRandomizerInitSeed;
		mRandomizerCurrentState = mRandomizerInitState;
	schnaps_StackTraceEndM("void simulator::Simulator::readRandomizerInfo(PACC::XML::ConstIterator)");
}
...
void Simulator::readEnvironmentOutput(PACC::XML::ConstIterator inIter) {
	...
	schnaps_StackTraceEndM("void simulator::Simulator::readEnvironmentOutput(PACC::XML::ConstIterator)");
}
...
void Simulator::readPopulationOutput(PACC::XML::ConstIterator inIter) {
	...
	schnaps_StackTraceEndM("void simulator::Simulator::readPopulationOutput(PACC::XML::ConstIterator)");
}
...
void Simulator::writeInput(PACC::XML::Streamer& ioStreamer, bool inIndent) const {
	schnaps_StackTraceBeginM();
		...
		ioStreamer.closeTag();
	schnaps_StackTraceEndM("void simulator::Simulator::writeInput(PACC::XML::Streamer&, bool)");
}
...
void Simulator::writeSimulation(PACC::XML::Streamer& ioStreamer, bool inIndent) const {
	schnaps_StackTraceBeginM();
		...
		ioStreamer.closeTag();
	schnaps_StackTraceEndM("void simulator::Simulator::writeSimulationPACC::XML::Streamer&, bool)");
}
...
void Simulator::writeOutput(PACC::XML::Streamer& ioStreamer, bool inIndent) const {
	schnaps_StackTraceBeginM();
		...
		ioStreamer.closeTag();
	schnaps_StackTraceEndM("void simulator::Simulator::writeOutput(PACC::XML::Streamer&, bool)");
}
...
void Simulator::writeRandomizerInfo(PACC::XML::Streamer& ioStreamer, bool inIndent) const {
	schnaps_StackTraceBeginM();
		...
		ioStreamer.closeTag();
	schnaps_StackTraceEndM("void simulator::Simulator::writeRandomizerInfo(PACC::XML::Streamer&, bool)");
}
...
void Simulator::writeEnvironmentOutput(PACC::XML::Streamer& ioStreamer, bool inIndent) const {
	schnaps_StackTraceBeginM();
		...
		ioStreamer.closeTag();
	schnaps_StackTraceEndM("void simulator::Simulator::writeEvironmentOutput(PACC::XML::Streamer&, bool)");
}
...
void Simulator::writePopulationOutput(PACC::XML::Streamer& ioStreamer, bool inIndent) const {
	schnaps_StackTraceBeginM();
		...
		ioStreamer.closeTag();
	schnaps_StackTraceEndM("void simulator::Simulator::writePopulationOutput(PACC::XML::Streamer&, bool)");
}
...
void Simulator::printInput(std::ostream& ioStream, unsigned int inLowerBound) const {
	...
	schnaps_StackTraceEndM("void simulator::Simulator::printInput(std::ostream&, unsigned int) const");
}
...
void Simulator::printOutput(std::ostream& ioStream) const {
	...
	schnaps_StackTraceEndM("void simulator::Simulator::printOutput(std::ostream&) const");
}
...
void Simulator::printSummary(std::ostream& ioStream) const {
	schnaps_StackTraceBeginM();
		...
		ioStreamer.closeTag();
	schnaps_StackTraceEndM("void simulator::Simulator::printSummary(std::ostream&) const");
}
