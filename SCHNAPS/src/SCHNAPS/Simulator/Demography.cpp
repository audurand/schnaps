...
void Demography::readWithSystem(PACC::XML::ConstIterator inIter, core::System& ioSystem) {
		...
		if (lFile.empty()) {
			...
			for (PACC::XML::ConstIterator lChild = inIter->getFirstChild(); lChild; lChild++) {
				if (lChild->getType() == PACC::XML::eData) {
					...
					this->push_back(std::pair<std::string, core::PrimitiveTree::Handle>(lLabel, new core::PrimitiveTree()));
	...
	schnaps_StackTraceEndM("void simulator::Demography::readWithSystem(PACC::XML::ConstIterator, core::System&)");
}
